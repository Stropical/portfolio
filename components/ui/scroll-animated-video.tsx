...
export type HeroScrollVideoProps = {
  // Optional custom hero object. If `hero.node` is provided it will be rendered
  // in place of the default headline markup. Other properties act as overrides
  // for the existing title/subtitle/meta/credits values.
  hero?: {
    node?: ReactNode;
    title?: ReactNode;
    subtitle?: ReactNode;
    ...
    >
      ...
      <div className="hsv-container" ref={headlineRef}>
        <div className="hsv-headline">
          {/* If a custom hero node is supplied render it directly. Otherwise
              render the default headline using either hero overrides or the
              component's title/subtitle/meta/credits props for compatibility. */}
          {hero?.node ? (
            hero.node
          ) : (
            <>
              <h1 className="hsv-title">{hero?.title ?? title}</h1>
              {hero?.subtitle ?? subtitle ? (
                <h2 className="hsv-subtitle">{hero?.subtitle ?? subtitle}</h2>
              ) : null}
              {hero?.meta ?? meta ? (
                <div className="hsv-meta">{hero?.meta ?? meta}</div>
              ) : null}
              {hero?.credits ?? credits ? (
                <div className="hsv-credits">{hero?.credits ?? credits}</div>
              ) : null}
            </>
          )}
        </div>
